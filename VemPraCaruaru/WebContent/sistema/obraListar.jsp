        	...
        	<table class="tabelaListar" id="tabelaListar">
              ...
              try {
            	  ...
            	  for (Obra obra : obras) {
					...
					String paginaAtivoInativo = obra.getAtivo() == 'S' ? "../ObraInativar?id=" + obra.getId() : "../ObraAtivar?id=" + obra.getId();
	                %>
	                <tr id="<%=cssClasse%>">
	                  ...
	                  <td style="text-align:center; vertical-align:top; padding-top:4px;"><a href="obraListarDetalhes.jsp?id=<%=obra.getId()%>" onclick="return hs.htmlExpand(this, { objectType: 'iframe', preserveContent: false, width: '700', height: '450'} )"><img src="../img/verDetalhes.png" alt="Visualizar detalhes da obra de ID <%=obra.getId()%>" title="Visualizar detalhes da obra de ID <%=obra.getId()%>" width="44" height="24"></a>&nbsp;<a href="obraListarFotos.jsp?id=<%=obra.getId()%>" onclick="return hs.htmlExpand(this, { objectType: 'iframe', preserveContent: false, width: '880', height: '600'} )"><img src="../img/fotos.png" alt="Visualizar fotos da obra de ID <%=obra.getId()%>" title="Visualizar fotos da obra de ID <%=obra.getId()%>" width="33" height="24"></a>&nbsp;<a href="<%=paginaAtivoInativo%>" target="recebeForm"><img src="../img/<%=imagemAtivarInativar %>" alt="<%=nomeAtivarInativar %> a obra de ID <%=obra.getId()%>" title="<%=nomeAtivarInativar %> a obra de ID <%=obra.getId()%>" width="24" height="24"></a>&nbsp;<a href="obraAlterar.jsp?id=<%=obra.getId()%>"><img src="../img/alterar.png" alt="Alterar a obra de ID <%=obra.getId()%>" title="Alterar a obra de ID <%=obra.getId()%>" width="24" height="24"></a></td>
	                </tr>
	                <%        
            	  }
              } catch (Exception e) {
