 ...
 */
@WebServlet("/ObraInativar")
public class ObraInativar extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
		    ...
		    if (id > 0){
		    	Fachada.getInstance().obraDeletar(id);
		    	out.println( "<script>parent.alert(\"Obra inativada com sucesso!\");</script>" );
			...
			e.printStackTrace();
			out.println( "<script>parent.alert(\"Ocorreu um erro inesperado ao inativar a obra!\");</script>" );
		}
	}
